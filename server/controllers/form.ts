...
export class Form {
    ...
    search(req, res) {
        logger.debug(`ReqId = "${req.headers['X-msgid']}": Form search method is called`);
        ...
        logger.debug(`ReqId = "${req.headers['X-msgid']}": Finding the data from Form database`)
        this.databaseSdk.find('form', searchObj)
            .then(data => {
                data = _.map(data.docs, doc => _.omit(doc, ['_id', '_rev']))
                if (data.length <= 0) {
                    logger.error(`ReqId = "${req.headers['X-msgid']}": Received empty data while searching with ${searchObj} in form database`)
                    res.status(404);
                ...
                logger.info(`ReqId = "${req.headers['X-msgid']}": Received data  from - form database`)
                return res.send(Response.success("api.form.read", resObj));
            })
            .catch(err => {
                logger.error(`ReqId = "${req.headers['X-msgid']}": Received error while searching in form database and err.message: ${err.message} ${err}`)
                if (err.status === 404) {
                    res.status(404)
                    return res.send(Response.error("api.form.read", 404));
                } else {

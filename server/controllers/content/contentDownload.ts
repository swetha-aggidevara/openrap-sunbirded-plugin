            ...
            try {
                ...
                if (_.indexOf(status, API_DOWNLOAD_STATUS.inprogress) !== -1) {
                    ...
                    if (!_.isEmpty(inprogressItems)) {
                        const downloadIds = _.map(inprogressItems, "id");
                        submitted = _.filter(submitted, (s) => _.indexOf(downloadIds, s.id) === -1);
                        ...
                        _.forEach(inprogressItems, (item) => {
                            const contentItem = _.find(inProgressDbData.docs, { downloadId: item.id });
                            inprogress.push({

...
export default class ContentManager {
    ...
    async startImport(fileName, x_msgId) {
        ...
        logger.info(` X-msgID = "${x_msgId}": File is unzipped and reading the manifest file and add basedir to manifest as content and folder name relative path`);
        ...
        if (items && _.isArray(items) &&
            ...
            if (parent) {
                ...
                logger.info(` X-msgID = "${x_msgId}": Hierarchy is created for the collection ${_.get(parent, 'identifier')}`)
                parent['children'] = children;
                logger.info(` X-msgID = "${x_msgId}": Adding metadata for parent ${_.get(parent, 'identifier')}`);
                                ...
                                if (err) {
                                    ...
                                    if (stats.isDirectory()) {
                                        ...
                                        if (!_.isEmpty(item)) {
                                            ...
                                            logger.info(` X-msgID = "${x_msgId}": created manifest for the file ${file}`);
                                        }
    ...
    createHierarchy(items: any[], parent: any, x_msgId?: any,tree?: any[]): any {
        ...
        logger.info(` X-msgID = "${x_msgId}": Child contents are fond for Parent: ${_.get(parent, 'identifier')}`);
        return tree;
    }
